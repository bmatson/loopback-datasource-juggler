...
function validateCustom(attr, conf, err, options, done) {
  ...
  if (done === undefined) { // called from synch validator, stick options on end
    conf.customValidator.call(this, err, options);
  } else {
    if (conf.customValidator.length === 3) {  // if they declared the validator with 3 params, they are expecting options
